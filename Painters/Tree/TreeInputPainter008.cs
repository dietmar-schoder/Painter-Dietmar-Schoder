...
{
    public class TreeInputPainter008 : IPainter
    {
        ...
        //private int[] _branchAngles = new int[] { 0, 180, 90 };
        //private int[] _branchAngles = new int[] { 0, 89, 179, 271 };
        //private int[] _branchAngles = new int[] { 0, 87, 177, 273 };
        ...
        private int _a1 = 16, _a2 = 225;
        private int _margin = 20;
        private int _branchLength = 25;
        private int _density = 4;
        private int _maxDepth = 500;
        private int _minTolerance = 1;
        private int _maxTolerance = 80;
        private int _minPenThickness = 2;
        private int _maxPenThickness = 100;
        private bool _bezier = false;
        private bool _curves = true;
        private bool _paintDots = false;
        private int _centerX, _centerY, _maxDistanceFromCenter;

        public TreeInputPainter008() { }
        ...
        public void DrawOnCanvas(ICanvas canvas)
        {
            ...
            _maxDistanceFromCenter = _centerX * _centerX + _centerY * _centerY;
            var startX = _centerX / _canvas.EnlargeFactor;
            var startY = _centerY / _canvas.EnlargeFactor;
            ...
            while (_pixelQueue.Count > 0)
            {
                while (_pixelQueue.Count > 0)
                {
                    ...
                    if (_bezier)
                    {
                        ...
                        using var pen = new Pen(Color.FromArgb(_rnd.Next(_a1, _a2), root.Color.R, root.Color.G, root.Color.B), _maxPenThickness);
        ...
        private bool TryGetNewChild(Node2 node, int angle, out Node2 newChild)
        {
            ...
            do
            {
                ...
                tolerance = GetDistanceFactor(_minTolerance, _maxTolerance, new PointF(x, y));
            }
            while (IsWithinCanvas(x, y) && AreaIsFree(x, y) && IsWithinTolerance(node.StartColor, x, y, tolerance));
            distance--;
        ...
        private void DrawTree(Node2 node)
        {
            ...
            var penThickness = GetDistanceFactor(_minPenThickness * 10, _maxPenThickness, _pixelPath.First());
            ...
            using var pen = new Pen(
                ...
                _rnd.Next(_minPenThickness, penThickness));
            pen.StartCap = LineCap.Round;
            pen.EndCap = LineCap.Round;
            pen.LineJoin = LineJoin.Round;
        ...
        private int GetDistanceFactor(int min, int max, PointF refPoint)
        {
            var distanceFromCenter = (refPoint.X - _centerX) * (refPoint.X - _centerX) + (refPoint.Y - _centerY) * (refPoint.Y - _centerY);
            return min + (int)((max - min) * distanceFromCenter / _maxDistanceFromCenter);
        }
        ...
        private bool IsWithinTolerance(Color color, int x, int y, int tolerance)
        {
            var testColor = _inputBitmap.GetPixel(x, y);
            var isWithinTolerance = (int)testColor.R > (int)(color.R - tolerance) && (int)testColor.R < (int)(color.R + tolerance);
