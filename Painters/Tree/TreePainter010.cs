...
{
    public class TreePainter010 : IPainter
    {
        ...
        private int[] _branchAngles = new int[] { 0, 10, -10 };
        ...
        public void DrawOnCanvas(ICanvas canvas)
        {
            ...
            //for (var j = 0; j < 1; j++)
            //{
            var startVector = new Vector(0, -_branchLength); //.Rotated(j * 0.3);
            _referenceBitmap = new Bitmap(canvas.Width, canvas.Height);
            var startX = canvas.Width / 2; // + j * _repeatOffset;
            var startY = canvas.Height / 2; // + j * _repeatOffset;
            ...
            var root = new Node2(pixel.X, pixel.Y, startVector);
            root.StartColor = Color.Black;
        ...
        {
            var branchAngles = new int[] { 0, 120, 240 };
            //var branchAngles = new int[] { 0, 90, 180, 270 };
            //var branchAngles = new int[] { 0, 27, 60, 89, 120, 152, 180, 212, 240, 273, 300, 330 };
            //var branchAngles = new int[] { 0, _rnd.Next(118, 122), 360 - _rnd.Next(118, 122) };
            currentDepth++;
            if (node.Children.Count == 0)
            {
                for (int i = 0; i < branchAngles.Length; i++)
                {
                    var angleIndex = _rnd.Next(0, branchAngles.Length);
                    if (!TryGetNewChild(node, branchAngles[angleIndex], out var newChild)) { continue; }
                    //if (!TryGetNewChild(node, branchAngles[i], out var newChild)) { continue; }
        ...
        private void PaintDot(Node2 node)
        {
            ...
            gr.FillEllipse(brush, new Rectangle(node.X - 15, node.Y - 15, 30, 30));
        }
        ...
        private void SetAreaToCovered(int x, int y)
        {
            ...
            gr.FillRectangle(brush, x - _density / 2, y - _density / 2, _density, _density);
        }
