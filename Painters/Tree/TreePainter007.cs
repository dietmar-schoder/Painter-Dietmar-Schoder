...
{
    public class TreePainter007 : IPainter
    {
                ...
                while (_pixelQueue.Count > 0)
                {
                    while (_pixelQueue.Count > 0)
                    {
                        ...
                        if (_bezier)
                        {
                            ...
                            using var pen = new Pen(Color.Black, 30);
        ...
        private bool TryGetNewChild(Node2 node, int angle, out Node2 newChild)
        {
            ...
            while (distance < 5 && IsWithinCanvas(x, y) && AreaIsFree(x, y)); // && IsWithinTolerance(node.StartColor, x, y)
            distance--;
